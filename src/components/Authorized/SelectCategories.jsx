...
function SelectCategories({ categories, selectedCategory, onChange, open }) {
  return (
              ...
              <Listbox.Options className="absolute z-10 mt-1 max-h-60 w-1/3 overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                {categories.map((category, idx) => (
                  <Listbox.Option
                    key={idx}
                    className={({ active }) =>
                      classNames(active ? 'bg-yellow-600 text-white' : 'text-gray-900', 'relative cursor-default select-none py-2 pl-8 pr-4 capitalize')
                    }
                    value={category.id}
                  >
                    {({ selected, active }) => (
                      <>
                        <span className={classNames(selected ? 'font-semibold' : 'font-normal', 'block truncate capitalize')}>{category.id}</span>
                        {selected ? (
                          <span className={classNames(active ? 'text-chocolate' : 'text-yellow-500', 'absolute inset-y-0 left-0 flex items-center pl-1.5 capitalize')}>
